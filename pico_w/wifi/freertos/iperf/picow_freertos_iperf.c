...
void main_task(__unused void *params) {
    ...
    printf("In main_task watchdod_enabale is enabled 4sec..\n");
	watchdog_enable(40000, 1);
	//while (wifi_connected) {
    cyw43_arch_enable_sta_mode();
    printf("Connecting to Wi-Fi...\n");
    if (cyw43_arch_wifi_connect_timeout_ms(WIFI_SSID, WIFI_PASSWORD, CYW43_AUTH_WPA2_AES_PSK, 70000)) {
        printf("failed to connect.\n");
        exit(1);
    } else {
        printf("Connected.\n");
        sleep_ms(1000);
        printf("calling init_pico_mqtt:\n");
        init_pico_mqtt();
    }
	//}	 
